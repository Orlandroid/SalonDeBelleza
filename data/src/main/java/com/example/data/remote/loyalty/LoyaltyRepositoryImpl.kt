...
class LoyaltyRepositoryImpl @Inject constructor(
    ...
    override suspend fun getPromotionCodes(userId: String): ApiResult<List<PromotionCode>> {
        return runCatching {
            ...
            val codes =
                snapshot.children.mapNotNull { it.getValue<PromotionCode>() }.filter { !it.used }
    ...
    override suspend fun getLoyaltyTransactions(userId: String): ApiResult<List<LoyaltyTransaction>> {
        return runCatching {
            val snapshot = loyaltyTransactionsRef.child(userId).get().await()
            val transactions = snapshot.children.mapNotNull { it.getValue<LoyaltyTransaction>() }
    ...
    override fun observeLoyalty(userId: String): Flow<Loyalty?> = callbackFlow {
        val listener = object : ValueEventListener {
            override fun onDataChange(snapshot: DataSnapshot) {
                trySend(snapshot.getValue<Loyalty>())
            }
        ...
        loyaltyRef.child(userId).addValueEventListener(listener)
        awaitClose { loyaltyRef.child(userId).removeEventListener(listener) }
    }
}
